...
LOGICAL_OR_OPERATOR:
    '||' { self.setType(CONCAT_PIPES_SYMBOL if self.isSqlModeActive(PipesAsConcat) else LOGICAL_OR_OPERATOR) }
;
BITWISE_OR_OPERATOR: '|';
...
ADDDATE_SYMBOL:                  A D D D A T E                               { self.setType(determineFunction(ADDDATE_SYMBOL)); }; // MYSQL-FUNC
...
BIT_AND_SYMBOL:                  B I T '_' A N D                             { self.setType(determineFunction(BIT_AND_SYMBOL)); }; // MYSQL-FUNC
BIT_OR_SYMBOL:                   B I T '_' O R                               { self.setType(determineFunction(BIT_OR_SYMBOL)); }; // MYSQL-FUNC
BIT_SYMBOL:                      B I T;                                      // MYSQL-FUNC
BIT_XOR_SYMBOL:                  B I T '_' X O R                             { self.setType(determineFunction(BIT_XOR_SYMBOL)); }; // MYSQL-FUNC
...
CAST_SYMBOL:                     C A S T                                     { self.setType(determineFunction(CAST_SYMBOL)); }; // SQL-2003-R
...
COUNT_SYMBOL:                    C O U N T                                   { self.setType(determineFunction(COUNT_SYMBOL)); }; // SQL-2003-N
CPU_SYMBOL:                      C P U;
CREATE_SYMBOL:                   C R E A T E;                                // SQL-2003-R
CROSS_SYMBOL:                    C R O S S;                                  // SQL-2003-R
CUBE_SYMBOL:                     C U B E;                                    // SQL-2003-R
CURDATE_SYMBOL:                  C U R D A T E                               { self.setType(determineFunction(CURDATE_SYMBOL)); }; // MYSQL-FUNC
CURRENT_SYMBOL:                  C U R R E N T                               {serverVersion >= 50604}?;
CURRENT_DATE_SYMBOL:
    C U R R E N T '_' D A T E                                                { self.setType(determineFunction(CURDATE_SYMBOL)); }
;                                                                            // Synonym, MYSQL-FUNC
CURRENT_TIME_SYMBOL:
    C U R R E N T '_' T I M E                                                { self.setType(determineFunction(CURTIME_SYMBOL)); }
;                                                                            // Synonym, MYSQL-FUNC
...
CURTIME_SYMBOL:                  C U R T I M E                               { self.setType(determineFunction(CURTIME_SYMBOL)); }; // MYSQL-FUNC
...
DATE_ADD_SYMBOL:                 D A T E '_' A D D                           { self.setType(determineFunction(DATE_ADD_SYMBOL)); };
DATE_SUB_SYMBOL:                 D A T E '_' S U B                           { self.setType(determineFunction(DATE_SUB_SYMBOL)); };
...
EXTRACT_SYMBOL:                  E X T R A C T                               { self.setType(determineFunction(EXTRACT_SYMBOL)); }; // SQL-2003-N
...
GROUP_CONCAT_SYMBOL:
    G R O U P '_' C O N C A T                                                { self.setType(determineFunction(GROUP_CONCAT_SYMBOL)); }
;
...
;
MAX_SYMBOL:                      M A X                                       { self.setType(determineFunction(MAX_SYMBOL)); }; // SQL-2003-N
...
MID_SYMBOL:                      M I D                                       { self.setType(determineFunction(SUBSTRING_SYMBOL)); }; // Synonym
...
MIN_SYMBOL:                      M I N                                       { self.setType(determineFunction(MIN_SYMBOL)); }; // SQL-2003-N
MODE_SYMBOL:                     M O D E;
...
NOT_SYMBOL:
    N O T                                                                    { self.setType(NOT2_SYMBOL if self.isSqlModeActive(HighNotPrecedence) else  NOT_SYMBOL) }
;                                                                            // SQL-2003-R
NOW_SYMBOL:                      N O W                                       { self.setType(determineFunction(NOW_SYMBOL)); };
...
PORT_SYMBOL:                     P O R T;
POSITION_SYMBOL:                 P O S I T I O N                             { self.setType(determineFunction(POSITION_SYMBOL)); }; // SQL-2003-N
...
SESSION_USER_SYMBOL:
    S E S S I O N '_' U S E R                                                { self.setType(determineFunction(USER_SYMBOL)); }
;                                                                            // Synonym
...
STDDEV_SAMP_SYMBOL:
    S T D D E V '_' S A M P                                                  { self.setType(determineFunction(STDDEV_SAMP_SYMBOL)); }
;                                                                            // SQL-2003-N
STDDEV_SYMBOL:                   S T D D E V                                 { self.setType(determineFunction(STD_SYMBOL)); }; // Synonym
STDDEV_POP_SYMBOL:               S T D D E V '_' P O P                       { self.setType(determineFunction(STD_SYMBOL)); }; // Synonym
STD_SYMBOL:                      S T D                                       { self.setType(determineFunction(STD_SYMBOL)); };
STOP_SYMBOL:                     S T O P;
...
SUBDATE_SYMBOL:                  S U B D A T E                               { self.setType(determineFunction(SUBDATE_SYMBOL)); };
SUBJECT_SYMBOL:                  S U B J E C T;
SUBPARTITIONS_SYMBOL:            S U B P A R T I T I O N S;
SUBPARTITION_SYMBOL:             S U B P A R T I T I O N;
SUBSTR_SYMBOL:                   S U B S T R                                 { self.setType(determineFunction(SUBSTRING_SYMBOL)); }; // Synonym
SUBSTRING_SYMBOL:                S U B S T R I N G                           { self.setType(determineFunction(SUBSTRING_SYMBOL)); }; // SQL-2003-N
SUM_SYMBOL:                      S U M                                       { self.setType(determineFunction(SUM_SYMBOL)); }; // SQL-2003-N
SUPER_SYMBOL:                    S U P E R;
SUSPEND_SYMBOL:                  S U S P E N D;
SWAPS_SYMBOL:                    S W A P S;
SWITCHES_SYMBOL:                 S W I T C H E S;
SYSDATE_SYMBOL:                  S Y S D A T E                               { self.setType(determineFunction(SYSDATE_SYMBOL)); };
SYSTEM_USER_SYMBOL:              S Y S T E M '_' U S E R                     { self.setType(determineFunction(USER_SYMBOL)); };
...
TRIM_SYMBOL:                     T R I M                                     { self.setType(determineFunction(TRIM_SYMBOL)); }; // SQL-2003-N
...
VARIANCE_SYMBOL:                 V A R I A N C E                             { self.setType(determineFunction(VARIANCE_SYMBOL)); };
VARYING_SYMBOL:                  V A R Y I N G;                              // SQL-2003-R
VAR_POP_SYMBOL:                  V A R '_' P O P                             { self.setType(determineFunction(VARIANCE_SYMBOL)); }; // Synonym
VAR_SAMP_SYMBOL:                 V A R '_' S A M P                           { self.setType(determineFunction(VAR_SAMP_SYMBOL)); };
...
UNDERSCORE_CHARSET: UNDERLINE_SYMBOL [a-z0-9]+ { self.setType(checkCharset(self.getText())); };
...
VERSION_COMMENT_START: ('/*!' DIGITS) (
        {checkVersion(self.getText())}? // Will set inVersionComment if the number matches.
        | .*? '*/'
    ) -> channel(HIDDEN)
;
