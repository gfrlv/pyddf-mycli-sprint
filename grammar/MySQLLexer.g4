...
LOGICAL_OR_OPERATOR:
    '||' { self.setType(self.CONCAT_PIPES_SYMBOL if self.isSqlModeActive(self.PipesAsConcat) else self.LOGICAL_OR_OPERATOR) }
;
BITWISE_OR_OPERATOR: '|';
...
JSON_SEPARATOR_SYMBOL:          '->' {self.server_version >= 50708}?;  // MYSQL
JSON_UNQUOTED_SEPARATOR_SYMBOL: '->>' {self.server_version >= 50713}?; // MYSQL
...
INT_NUMBER: DIGITS { self.setType(self.determineNumericType(self.getText())) };
...
ACCOUNT_SYMBOL:                  A C C O U N T                               {self.server_version >= 50707}?;
ACTION_SYMBOL:                   A C T I O N;                                // SQL-2003-N
ADD_SYMBOL:                      A D D;                                      // SQL-2003-R
ADDDATE_SYMBOL:                  A D D D A T E                               { self.setType(self.determineFunction(self.ADDDATE_SYMBOL)) }; // MYSQL-FUNC
...
ALWAYS_SYMBOL:                   A L W A Y S                                 {self.server_version >= 50707}?;
ANALYSE_SYMBOL:                  A N A L Y S E                               {self.server_version < 80000}?;
...
AT_SYMBOL:                       A T;
AUTHORS_SYMBOL:                  A U T H O R S                               {self.server_version < 50700}?;
...
BIT_AND_SYMBOL:                  B I T '_' A N D                             { self.setType(self.determineFunction(self.BIT_AND_SYMBOL)) }; // MYSQL-FUNC
BIT_OR_SYMBOL:                   B I T '_' O R                               { self.setType(self.determineFunction(self.BIT_OR_SYMBOL)) }; // MYSQL-FUNC
BIT_SYMBOL:                      B I T;                                      // MYSQL-FUNC
BIT_XOR_SYMBOL:                  B I T '_' X O R                             { self.setType(self.determineFunction(self.BIT_XOR_SYMBOL)) }; // MYSQL-FUNC
...
CAST_SYMBOL:                     C A S T                                     { self.setType(self.determineFunction(self.CAST_SYMBOL)) }; // SQL-2003-R
...
CHANNEL_SYMBOL:                  C H A N N E L                               {self.server_version >= 50706}?;
...
COMPRESSION_SYMBOL:              C O M P R E S S I O N                       {self.server_version >= 50707}?;
...
CONTRIBUTORS_SYMBOL:             C O N T R I B U T O R S                     {self.server_version < 50700}?;
CONVERT_SYMBOL:                  C O N V E R T;                              // SQL-2003-N
COUNT_SYMBOL:                    C O U N T                                   { self.setType(self.determineFunction(self.COUNT_SYMBOL)) }; // SQL-2003-N
CPU_SYMBOL:                      C P U;
CREATE_SYMBOL:                   C R E A T E;                                // SQL-2003-R
CROSS_SYMBOL:                    C R O S S;                                  // SQL-2003-R
CUBE_SYMBOL:                     C U B E;                                    // SQL-2003-R
CURDATE_SYMBOL:                  C U R D A T E                               { self.setType(self.determineFunction(self.CURDATE_SYMBOL)) }; // MYSQL-FUNC
CURRENT_SYMBOL:                  C U R R E N T                               {self.server_version >= 50604}?;
CURRENT_DATE_SYMBOL:
    C U R R E N T '_' D A T E                                                { self.setType(self.determineFunction(self.CURDATE_SYMBOL)) }
;                                                                            // Synonym, MYSQL-FUNC
CURRENT_TIME_SYMBOL:
    C U R R E N T '_' T I M E                                                { self.setType(self.determineFunction(self.CURTIME_SYMBOL)) }
;                                                                            // Synonym, MYSQL-FUNC
...
CURTIME_SYMBOL:                  C U R T I M E                               { self.setType(self.determineFunction(self.CURTIME_SYMBOL)) }; // MYSQL-FUNC
...
DATE_ADD_SYMBOL:                 D A T E '_' A D D                           { self.setType(self.determineFunction(self.DATE_ADD_SYMBOL)) };
DATE_SUB_SYMBOL:                 D A T E '_' S U B                           { self.setType(self.determineFunction(self.DATE_SUB_SYMBOL)) };
...
DEFAULT_AUTH_SYMBOL:             D E F A U L T '_' A U T H                   {self.server_version >= 50604}?; // Internal
...
DES_KEY_FILE_SYMBOL:             D E S '_' K E Y '_' F I L E                 {self.server_version < 80000}?;
...
ENCRYPTION_SYMBOL:               E N C R Y P T I O N                         {self.server_version >= 50711}?;
...
EXPIRE_SYMBOL:                   E X P I R E                                 {self.server_version >= 50606}?;
EXPLAIN_SYMBOL:                  E X P L A I N;                              // SQL-2003-R
EXPORT_SYMBOL:                   E X P O R T                                 {self.server_version >= 50606}?;
EXTENDED_SYMBOL:                 E X T E N D E D;
EXTENT_SIZE_SYMBOL:              E X T E N T '_' S I Z E;
EXTRACT_SYMBOL:                  E X T R A C T                               { self.setType(self.determineFunction(self.EXTRACT_SYMBOL)) }; // SQL-2003-N
...
FILE_SYMBOL:                     F I L E;
FILE_BLOCK_SIZE_SYMBOL:          F I L E '_' B L O C K '_' S I Z E           {self.server_version >= 50707}?;
FILTER_SYMBOL:                   F I L T E R                                 {self.server_version >= 50700}?;
...
FOLLOWS_SYMBOL:                  F O L L O W S                               {self.server_version >= 50700}?;
...
GET_SYMBOL:                      G E T                                       {self.server_version >= 50604}?;
GENERAL_SYMBOL:                  G E N E R A L;
GENERATED_SYMBOL:                G E N E R A T E D                           {self.server_version >= 50707}?;
GROUP_REPLICATION_SYMBOL:        G R O U P '_' R E P L I C A T I O N         {self.server_version >= 50707}?;
...
GROUP_CONCAT_SYMBOL:
    G R O U P '_' C O N C A T                                                { self.setType(self.determineFunction(self.GROUP_CONCAT_SYMBOL)) }
;
...
INSTANCE_SYMBOL:                 I N S T A N C E                             {self.server_version >= 50713}?;
...
JSON_SYMBOL:                     J S O N                                     {self.server_version >= 50708}?; // MYSQL
KEYS_SYMBOL:                     K E Y S;
...
MASTER_AUTO_POSITION_SYMBOL:     M A S T E R '_' A U T O '_' P O S I T I O N {self.server_version >= 50605}?;
MASTER_BIND_SYMBOL:              M A S T E R '_' B I N D                     {self.server_version >= 50602}?;
...
MASTER_RETRY_COUNT_SYMBOL:       M A S T E R '_' R E T R Y '_' C O U N T     {self.server_version >= 50601}?;
...
MASTER_SSL_CRL_SYMBOL:           M A S T E R '_' S S L '_' C R L             {self.server_version >= 50603}?;
MASTER_SSL_CRLPATH_SYMBOL:       M A S T E R '_' S S L '_' C R L P A T H     {self.server_version >= 50603}?;
...
MASTER_TLS_VERSION_SYMBOL:       M A S T E R '_' T L S '_' V E R S I O N     {self.server_version >= 50713}?;
...
MAX_STATEMENT_TIME_SYMBOL:
    M A X '_' S T A T E M E N T '_' T I M E                                  {50704 < self.server_version and self.server_version < 50708}?
;
MAX_SYMBOL:                      M A X                                       { self.setType(self.determineFunction(self.MAX_SYMBOL)) }; // SQL-2003-N
...
MID_SYMBOL:                      M I D                                       { self.setType(self.determineFunction(self.SUBSTRING_SYMBOL)) }; // Synonym
...
MIN_SYMBOL:                      M I N                                       { self.setType(self.determineFunction(self.MIN_SYMBOL)) }; // SQL-2003-N
MODE_SYMBOL:                     M O D E;
...
NEG_SYMBOL:                      N E G;
NEVER_SYMBOL:                    N E V E R                                   {self.server_version >= 50704}?;
...
NONBLOCKING_SYMBOL:              N O N B L O C K I N G                       {50700 < self.server_version and self.server_version < 50706}?;
NOT_SYMBOL:
    N O T                                                                    { self.setType(self.NOT2_SYMBOL if self.isSqlModeActive(self.HighNotPrecedence) else  self.NOT_SYMBOL) }
;                                                                            // SQL-2003-R
NOW_SYMBOL:                      N O W                                       { self.setType(self.determineFunction(self.NOW_SYMBOL)) };
...
NUMBER_SYMBOL:                   N U M B E R                                 {self.server_version >= 50606}?;
...
OLD_PASSWORD_SYMBOL:             O L D '_' P A S S W O R D                   {self.server_version < 50706}?;
ON_SYMBOL:                       O N;                                        // SQL-2003-R
ONE_SYMBOL:                      O N E;
ONLINE_SYMBOL:                   O N L I N E;
ONLY_SYMBOL:                     O N L Y                                     {self.server_version >= 50605}?;
OPEN_SYMBOL:                     O P E N;                                    // SQL-2003-R
OPTIMIZE_SYMBOL:                 O P T I M I Z E;
OPTIMIZER_COSTS_SYMBOL:          O P T I M I Z E R '_' C O S T S             {self.server_version >= 50706}?;
...
PLUGIN_DIR_SYMBOL:               P L U G I N '_' D I R                       {self.server_version >= 50604}?; // Internal
PLUGIN_SYMBOL:                   P L U G I N;
POINT_SYMBOL:                    P O I N T;
POLYGON_SYMBOL:                  P O L Y G O N;                              // MYSQL
PORT_SYMBOL:                     P O R T;
POSITION_SYMBOL:                 P O S I T I O N                             { self.setType(self.determineFunction(self.POSITION_SYMBOL)) }; // SQL-2003-N
PRECEDES_SYMBOL:                 P R E C E D E S                             {self.server_version >= 50700}?;
...
REDOFILE_SYMBOL:                 R E D O F I L E                             {self.server_version < 80000}?;
...
REPLICATE_DO_DB_SYMBOL:          R E P L I C A T E '_' D O '_' D B           {self.server_version >= 50700}?;
REPLICATE_IGNORE_DB_SYMBOL:      R E P L I C A T E '_' I G N O R E '_' D B   {self.server_version >= 50700}?;
REPLICATE_DO_TABLE_SYMBOL:       R E P L I C A T E '_' D O '_' T A B L E     {self.server_version >= 50700}?;
REPLICATE_IGNORE_TABLE_SYMBOL:
    R E P L I C A T E '_' I G N O R E '_' T A B L E                          {self.server_version >= 50700}?
;
REPLICATE_WILD_DO_TABLE_SYMBOL:
    R E P L I C A T E '_' W I L D '_' D O '_' T A B L E                      {self.server_version >= 50700}?
;
REPLICATE_WILD_IGNORE_TABLE_SYMBOL:
    R E P L I C A T E '_' W I L D '_' I G N O R E '_' T A B L E              {self.server_version >= 50700}?
;
REPLICATE_REWRITE_DB_SYMBOL:     R E P L I C A T E '_' R E W R I T E '_' D B {self.server_version >= 50700}?;
...
ROTATE_SYMBOL:                   R O T A T E                                 {self.server_version >= 50713}?;
...
SESSION_USER_SYMBOL:
    S E S S I O N '_' U S E R                                                { self.setType(self.determineFunction(USER_SYMBOL)) }
;                                                                            // Synonym
...
SQL_AFTER_MTS_GAPS_SYMBOL:
    S Q L '_' A F T E R '_' M T S '_' G A P S                                {self.server_version >= 50606}?
;                                                                            // MYSQL
...
SQL_CACHE_SYMBOL:                S Q L '_' C A C H E                         {self.server_version < 80000}?;
...
SSL_SYMBOL:                      S S L;
STACKED_SYMBOL:                  S T A C K E D                               {self.server_version >= 50700}?;
...
STDDEV_SAMP_SYMBOL:
    S T D D E V '_' S A M P                                                  { self.setType(self.determineFunction(self.STDDEV_SAMP_SYMBOL)) }
;                                                                            // SQL-2003-N
STDDEV_SYMBOL:                   S T D D E V                                 { self.setType(self.determineFunction(self.STD_SYMBOL)) }; // Synonym
STDDEV_POP_SYMBOL:               S T D D E V '_' P O P                       { self.setType(self.determineFunction(self.STD_SYMBOL)) }; // Synonym
STD_SYMBOL:                      S T D                                       { self.setType(self.determineFunction(self.STD_SYMBOL)) };
STOP_SYMBOL:                     S T O P;
STORAGE_SYMBOL:                  S T O R A G E;
STORED_SYMBOL:                   S T O R E D                                 {self.server_version >= 50707}?;
...
SUBDATE_SYMBOL:                  S U B D A T E                               { self.setType(self.determineFunction(self.SUBDATE_SYMBOL)) };
SUBJECT_SYMBOL:                  S U B J E C T;
SUBPARTITIONS_SYMBOL:            S U B P A R T I T I O N S;
SUBPARTITION_SYMBOL:             S U B P A R T I T I O N;
SUBSTR_SYMBOL:                   S U B S T R                                 { self.setType(self.determineFunction(self.SUBSTRING_SYMBOL)) }; // Synonym
SUBSTRING_SYMBOL:                S U B S T R I N G                           { self.setType(self.determineFunction(self.SUBSTRING_SYMBOL)) }; // SQL-2003-N
SUM_SYMBOL:                      S U M                                       { self.setType(self.determineFunction(self.SUM_SYMBOL)) }; // SQL-2003-N
SUPER_SYMBOL:                    S U P E R;
SUSPEND_SYMBOL:                  S U S P E N D;
SWAPS_SYMBOL:                    S W A P S;
SWITCHES_SYMBOL:                 S W I T C H E S;
SYSDATE_SYMBOL:                  S Y S D A T E                               { self.setType(self.determineFunction(self.SYSDATE_SYMBOL)) };
SYSTEM_USER_SYMBOL:              S Y S T E M '_' U S E R                     { self.setType(self.determineFunction(self.USER_SYMBOL)) };
TABLES_SYMBOL:                   T A B L E S;
TABLESPACE_SYMBOL:               T A B L E S P A C E;
TABLE_REF_PRIORITY_SYMBOL:       T A B L E '_' R E F '_' P R I O R I T Y     {self.server_version < 80000}?;
...
TRIM_SYMBOL:                     T R I M                                     { self.setType(self.determineFunction(self.TRIM_SYMBOL)) }; // SQL-2003-N
...
VALIDATION_SYMBOL:               V A L I D A T I O N                         {self.server_version >= 50706}?;
...
VARIANCE_SYMBOL:                 V A R I A N C E                             { self.setType(self.determineFunction(self.VARIANCE_SYMBOL)) };
VARYING_SYMBOL:                  V A R Y I N G;                              // SQL-2003-R
VAR_POP_SYMBOL:                  V A R '_' P O P                             { self.setType(self.determineFunction(self.VARIANCE_SYMBOL)) }; // Synonym
VAR_SAMP_SYMBOL:                 V A R '_' S A M P                           { self.setType(self.determineFunction(self.VAR_SAMP_SYMBOL)) };
VIEW_SYMBOL:                     V I E W;                                    // SQL-2003-N
VIRTUAL_SYMBOL:                  V I R T U A L                               {self.server_version >= 50707}?;
WAIT_SYMBOL:                     W A I T;
...
X509_SYMBOL:                     X '509';
XA_SYMBOL:                       X A;
XID_SYMBOL:                      X I D                                       {self.server_version >= 50704}?;
XML_SYMBOL:                      X M L;
XOR_SYMBOL:                      X O R;
...
*/
PERSIST_SYMBOL:                  P E R S I S T                               {self.server_version >= 80000}?;
ROLE_SYMBOL:                     R O L E                                     {self.server_version >= 80000}?; // SQL-1999-R
ADMIN_SYMBOL:                    A D M I N                                   {self.server_version >= 80000}?; // SQL-1999-R
INVISIBLE_SYMBOL:                I N V I S I B L E                           {self.server_version >= 80000}?;
VISIBLE_SYMBOL:                  V I S I B L E                               {self.server_version >= 80000}?;
EXCEPT_SYMBOL:                   E X C E P T                                 {self.server_version >= 80000}?; // SQL-1999-R
COMPONENT_SYMBOL:                C O M P O N E N T                           {self.server_version >= 80000}?; // MYSQL
RECURSIVE_SYMBOL:                R E C U R S I V E                           {self.server_version >= 80000}?; // SQL-1999-R
JSON_OBJECTAGG_SYMBOL:           J S O N '_' O B J E C T A G G               {self.server_version >= 80000}?; // SQL-2015-R
JSON_ARRAYAGG_SYMBOL:            J S O N '_' A R R A Y A G G                 {self.server_version >= 80000}?; // SQL-2015-R
OF_SYMBOL:                       O F                                         {self.server_version >= 80000}?; // SQL-1999-R
SKIP_SYMBOL:                     S K I P                                     {self.server_version >= 80000}?; // MYSQL
LOCKED_SYMBOL:                   L O C K E D                                 {self.server_version >= 80000}?; // MYSQL
NOWAIT_SYMBOL:                   N O W A I T                                 {self.server_version >= 80000}?; // MYSQL
GROUPING_SYMBOL:                 G R O U P I N G                             {self.server_version >= 80000}?; // SQL-2011-R
PERSIST_ONLY_SYMBOL:             P E R S I S T '_' O N L Y                   {self.server_version >= 80000}?; // MYSQL
HISTOGRAM_SYMBOL:                H I S T O G R A M                           {self.server_version >= 80000}?; // MYSQL
BUCKETS_SYMBOL:                  B U C K E T S                               {self.server_version >= 80000}?; // MYSQL
REMOTE_SYMBOL:                   R E M O T E                                 {self.server_version >= 80003 and self.server_version < 80014}?; // MYSQL
CLONE_SYMBOL:                    C L O N E                                   {self.server_version >= 80000}?; // MYSQL
CUME_DIST_SYMBOL:                C U M E '_' D I S T                         {self.server_version >= 80000}?; // SQL-2003-R
DENSE_RANK_SYMBOL:               D E N S E '_' R A N K                       {self.server_version >= 80000}?; // SQL-2003-R
EXCLUDE_SYMBOL:                  E X C L U D E                               {self.server_version >= 80000}?; // SQL-2003-N
FIRST_VALUE_SYMBOL:              F I R S T '_' V A L U E                     {self.server_version >= 80000}?; // SQL-2011-R
FOLLOWING_SYMBOL:                F O L L O W I N G                           {self.server_version >= 80000}?; // SQL-2003-N
GROUPS_SYMBOL:                   G R O U P S                                 {self.server_version >= 80000}?; // SQL-2011-R
LAG_SYMBOL:                      L A G                                       {self.server_version >= 80000}?; // SQL-2011-R
LAST_VALUE_SYMBOL:               L A S T '_' V A L U E                       {self.server_version >= 80000}?; // SQL-2011-R
LEAD_SYMBOL:                     L E A D                                     {self.server_version >= 80000}?; // SQL-2011-R
NTH_VALUE_SYMBOL:                N T H '_' V A L U E                         {self.server_version >= 80000}?; // SQL-2011-R
NTILE_SYMBOL:                    N T I L E                                   {self.server_version >= 80000}?; // SQL-2011-R
NULLS_SYMBOL:                    N U L L S                                   {self.server_version >= 80000}?; // SQL-2003-N
OTHERS_SYMBOL:                   O T H E R S                                 {self.server_version >= 80000}?; // SQL-2003-N
OVER_SYMBOL:                     O V E R                                     {self.server_version >= 80000}?; // SQL-2003-R
PERCENT_RANK_SYMBOL:             P E R C E N T '_' R A N K                   {self.server_version >= 80000}?; // SQL-2003-R
PRECEDING_SYMBOL:                P R E C E D I N G                           {self.server_version >= 80000}?; // SQL-2003-N
RANK_SYMBOL:                     R A N K                                     {self.server_version >= 80000}?; // SQL-2003-R
RESPECT_SYMBOL:                  R E S P E C T                               {self.server_version >= 80000}?; // SQL_2011-N
ROW_NUMBER_SYMBOL:               R O W '_' N U M B E R                       {self.server_version >= 80000}?; // SQL-2003-R
TIES_SYMBOL:                     T I E S                                     {self.server_version >= 80000}?; // SQL-2003-N
UNBOUNDED_SYMBOL:                U N B O U N D E D                           {self.server_version >= 80000}?; // SQL-2003-N
WINDOW_SYMBOL:                   W I N D O W                                 {self.server_version >= 80000}?; // SQL-2003-R
EMPTY_SYMBOL:                    E M P T Y                                   {self.server_version >= 80000}?; // SQL-2016-R
JSON_TABLE_SYMBOL:               J S O N '_' T A B L E                       {self.server_version >= 80000}?; // SQL-2016-R
NESTED_SYMBOL:                   N E S T E D                                 {self.server_version >= 80000}?; // SQL-2016-N
ORDINALITY_SYMBOL:               O R D I N A L I T Y                         {self.server_version >= 80000}?; // SQL-2003-N
PATH_SYMBOL:                     P A T H                                     {self.server_version >= 80000}?; // SQL-2003-N
HISTORY_SYMBOL:                  H I S T O R Y                               {self.server_version >= 80000}?; // MYSQL
REUSE_SYMBOL:                    R E U S E                                   {self.server_version >= 80000}?; // MYSQL
SRID_SYMBOL:                     S R I D                                     {self.server_version >= 80000}?; // MYSQL
THREAD_PRIORITY_SYMBOL:          T H R E A D '_' P R I O R I T Y             {self.server_version >= 80000}?; // MYSQL
RESOURCE_SYMBOL:                 R E S O U R C E                             {self.server_version >= 80000}?; // MYSQL
SYSTEM_SYMBOL:                   S Y S T E M                                 {self.server_version >= 80000}?; // SQL-2003-R
VCPU_SYMBOL:                     V C P U                                     {self.server_version >= 80000}?; // MYSQL
MASTER_PUBLIC_KEY_PATH_SYMBOL:
    M A S T E R '_' P U B L I C '_' K E Y '_' P A T H                        {self.server_version >= 80000}?
;                                                                            // MYSQL
GET_MASTER_PUBLIC_KEY_SYMBOL:
    G E T '_' M A S T E R '_' P U B L I C '_' K E Y '_' S Y M                {self.server_version >= 80000}?
;                                                                            // MYSQL
RESTART_SYMBOL:                  R E S T A R T                               {self.server_version >= 80011}?; // SQL-2003-N
DEFINITION_SYMBOL:               D E F I N I T I O N                         {self.server_version >= 80011}?; // MYSQL
DESCRIPTION_SYMBOL:              D E S C R I P T I O N                       {self.server_version >= 80011}?; // MYSQL
ORGANIZATION_SYMBOL:             O R G A N I Z A T I O N                     {self.server_version >= 80011}?; // MYSQL
REFERENCE_SYMBOL:                R E F E R E N C E                           {self.server_version >= 80011}?; // MYSQL

OPTIONAL_SYMBOL:                 O P T I O N A L                             {self.server_version >= 80013}?; // MYSQL
SECONDARY_SYMBOL:                S E C O N D A R Y                           {self.server_version >= 80013}?; // MYSQL
SECONDARY_ENGINE_SYMBOL:         S E C O N D A R Y '_' E N G I N E           {self.server_version >= 80013}?; // MYSQL
SECONDARY_LOAD_SYMBOL:           S E C O N D A R Y '_' L O A D               {self.server_version >= 80013}?; // MYSQL
SECONDARY_UNLOAD_SYMBOL:         S E C O N D A R Y '_' U N L O A D           {self.server_version >= 80013}?; // MYSQL

ACTIVE_SYMBOL:                   A C T I V E                                 {self.server_version >= 80014}?; // MYSQL
INACTIVE_SYMBOL:                 I N A C T I V E                             {self.server_version >= 80014}?; // MYSQL
LATERAL_SYMBOL:                  L A T E R A L                               {self.server_version >= 80014}?; // SQL-2003-R
RETAIN_SYMBOL:                   R E T A I N                                 {self.server_version >= 80014}?; // MYSQL
OLD_SYMBOL:                      O L D                                       {self.server_version >= 80014}?; // SQL-2003-R

NETWORK_NAMESPACE_SYMBOL:        N E T W O R K '_' N A M E S P A C E         {self.server_version >= 80017}?; // MYSQL
ENFORCED_SYMBOL:                 E N F O R C E D                             {self.server_version >= 80017}?; // SQL-2003-N
ARRAY_SYMBOL:                    A R R A Y                                   {self.server_version >= 80017}?; // SQL-2003-R
OJ_SYMBOL:                       O J                                         {self.server_version >= 80017}?; // ODBC
MEMBER_SYMBOL:                   M E M B E R                                 {self.server_version >= 80017}?; // SQL-2003-R

RANDOM_SYMBOL:                   R A N D O M                                 {self.server_version >= 80018}?; // MYSQL
MASTER_COMPRESSION_ALGORITHM_SYMBOL:
    M A S T E R '_' C O M P R E S S I O N '_' A L G O R I T H M              {self.server_version >= 80018}?
;                                                                            // MYSQL
MASTER_ZSTD_COMPRESSION_LEVEL_SYMBOL:
    M A S T E R '_' Z S T D '_' C O M P R E S S I O N '_' L E V E L          {self.server_version >= 80018}?
;                                                                            // MYSQL
PRIVILEGE_CHECKS_USER_SYMBOL:
    P R I V I L E G E '_' C H E C K S '_' U S E R                            {self.server_version >= 80018}?
;                                                                            // MYSQL
MASTER_TLS_CIPHERSUITES_SYMBOL:
    M A S T E R '_' T L S '_' C I P H E R S U I T E S                        {self.server_version >= 80018}?
;                                                                            // MYSQL

REQUIRE_ROW_FORMAT_SYMBOL:
    R E Q U I R E '_' R O W '_' F O R M A T                                  {self.server_version >= 80019}?
;                                                                            // MYSQL
PASSWORD_LOCK_TIME_SYMBOL:
    P A S S W O R D '_' L O C K '_' T I M E                                  {self.server_version >= 80019}?
;                                                                            // MYSQL
FAILED_LOGIN_ATTEMPTS_SYMBOL:
    F A I L E D '_' L O G I N '_' A T T E M P T S                            {self.server_version >= 80019}?
;                                                                            // MYSQL
REQUIRE_TABLE_PRIMARY_KEY_CHECK_SYMBOL:
    R E Q U I R E '_' T A B L E '_' P R I M A R Y '_' K E Y '_' C H E C K    {self.server_version >= 80019}?
;                                                                            // MYSQL
STREAM_SYMBOL:                   S T R E A M                                 {self.server_version >= 80019}?; // MYSQL
OFF_SYMBOL:                      O F F                                       {self.server_version >= 80019}?; // SQL-1999-R
...
UNDERSCORE_CHARSET: UNDERLINE_SYMBOL [a-z0-9]+ { self.setType(self.checkCharset(self.getText())) };
...
// TODO check syntax
BACK_TICK_QUOTED_ID: BACK_TICK (({not self.isSqlModeActive(self.NoBackslashEscapes)}? '\\')? .)*? BACK_TICK;

// TODO check syntax
DOUBLE_QUOTED_TEXT: (
        DOUBLE_QUOTE (({not self.isSqlModeActive(self.NoBackslashEscapes)}? '\\' .)? .)*? DOUBLE_QUOTE
    )+
;

// TODO check syntax
SINGLE_QUOTED_TEXT: (
        SINGLE_QUOTE (({not self.isSqlModeActive(self.NoBackslashEscapes)}? '\\')? .)*? SINGLE_QUOTE
    )+
;
...
VERSION_COMMENT_START: ('/*!' DIGITS) (
        {self.checkVersion(self.getText())}? // Will set inVersionComment if the number matches.
        | .*? '*/'
    ) -> channel(HIDDEN)
;
